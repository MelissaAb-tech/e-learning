...
class Chapitre
{
    ...
    public function estDejaVu($user_id, $chapitre_id)
    {
        $stmt = $this->db->prepare("SELECT COUNT(*) FROM chapitre_progression WHERE user_id = ? AND chapitre_id = ?");
        $stmt->execute([$user_id, $chapitre_id]);
        return $stmt->fetchColumn() > 0;
    }
    public function supprimerVu($user_id, $chapitre_id)
    {
        $stmt = $this->db->prepare("DELETE FROM chapitre_progression WHERE user_id = ? AND chapitre_id = ?");
        return $stmt->execute([$user_id, $chapitre_id]);
    }
}
